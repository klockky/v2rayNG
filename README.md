...
В этом форке **klockky**: закрыт вектор с **беспарольным локальным SOCKS** на `127.0.0.1` (новые логин/пароль на каждый старт VPN, те же данные уходят в hev и в ядро). Добавлены **подмена/настройка HWID и связанных заголовков** для запросов подписок (модуль DeviceKit) и **расшифровка подписок по ссылкам `happ://`** (включая варианты **crypto1–crypto4**). Обычное включение VPN **без ручного ввода прокси** в браузере.
