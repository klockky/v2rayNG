package com.v2ray.ang.util

import android.os.Build
import com.v2ray.ang.AngApplication
import com.v2ray.ang.AppConfig
import org.json.JSONArray
import org.json.JSONObject
import java.io.ByteArrayOutputStream
import java.security.KeyFactory
import java.security.PrivateKey
import java.security.spec.PKCS8EncodedKeySpec
import javax.crypto.Cipher
import javax.crypto.spec.IvParameterSpec
import javax.crypto.spec.SecretKeySpec
import android.util.Base64

/**
 * Decrypts happ:// deep-link subscription URLs.
 *
 * Algorithm and 4 PKCS#1 RSA private keys (crypt..crypt4) are derived from
 * https://github.com/LeeeeT/happ-decryptor (MIT-licensed). The 34 PKCS#8
 * keys for crypt5 plus selector table live in assets/happ/ — loaded once
 * and cached.
 *
 * Coverage:
 *   - crypt, crypt2, crypt3, crypt4: RSA-PKCS1v15. Works on every API
 *     level the app supports (24+).
 *   - crypt5: RSA-PKCS1v15 + ChaCha20-Poly1305. JCE exposes ChaCha20-Poly1305
 *     starting from Android 9 (API 28); on 24..27 we return null.
 */
object HappDecryptor {

    fun tryDecrypt(rawUrl: String?): String? {
        if (rawUrl.isNullOrBlank()) return null
        if (!rawUrl.startsWith("happ://", ignoreCase = true)) return null
        return try {
            decrypt(rawUrl)
        } catch (t: Throwable) {
            android.util.Log.w(AppConfig.TAG, "happ decrypt failed: ${t.message}")
            null
        }
    }

    @Throws(Exception::class)
    private fun decrypt(rawUrl: String): String? {
        val path = rawUrl.substring("happ://".length)
        return when {
            path.startsWith("crypt5/") -> decryptCrypt5(path.substring(7))
            path.startsWith("crypt4/") -> decryptRsaOnly(3, path.substring(7))
            path.startsWith("crypt3/") -> decryptRsaOnly(2, path.substring(7))
            path.startsWith("crypt2/") -> decryptRsaOnly(1, path.substring(7))
            path.startsWith("crypt/")  -> decryptRsaOnly(0, path.substring(6))
            else -> null
        }
    }

    // -- crypt..crypt4 -------------------------------------------------------

    private fun decryptRsaOnly(idx: Int, payload: String): String {
        val privateKey = pkcs1Keys[idx]
        val cipherBytes = b64DecodeUrlSafe(payload)
        val keySize = (privateKey.modulusBitLength + 7) / 8

        val cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding")
        cipher.init(Cipher.DECRYPT_MODE, privateKey.key)

        val out = ByteArrayOutputStream()
        var off = 0
        while (off < cipherBytes.size) {
            val end = minOf(off + keySize, cipherBytes.size)
            out.write(cipher.doFinal(cipherBytes, off, end - off))
            off = end
        }
        return out.toString("UTF-8")
    }

    // -- crypt5 --------------------------------------------------------------

    private fun decryptCrypt5(payload: String): String? {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.P) return null
        if (payload.length < 20) return null

        val n = payload.substring(18, 20).toIntOrNull() ?: return null
        val nonceStr = extractNonce(payload)
        val urlB64 = extractUrlB64(payload, n)
        val encStr = extractEncStr(payload, n)

        val trailingStart = encStr.trimEnd('=').length
        val eqIdx = encStr.indexOf('=')
        val rsaB64Raw = if (eqIdx in 0 until trailingStart) encStr.substring(eqIdx + 1) else encStr
        val rsaCipher = b64DecodeUrlSafe(rsaB64Raw.trimEnd('='))

        val selector = deriveSelector(GETHELP_ARG)
        val pkcs8B64 = expandedRsaKeyFor(selector) ?: return null
        val privateKey = parsePkcs8RsaKey(pkcs8B64)

        val rsa = Cipher.getInstance("RSA/ECB/PKCS1Padding")
        rsa.init(Cipher.DECRYPT_MODE, privateKey)
        val rsaPlain = rsa.doFinal(rsaCipher) // 44-byte base64 ASCII

        val chachaKey = b64DecodeUrlSafe(String(swapPairs(rsaPlain), Charsets.ISO_8859_1))
        if (chachaKey.size != 32) return null

        val nonce = nonceStr.toByteArray(Charsets.ISO_8859_1)
        if (nonce.size != 12) return null
        val ciphertext = b64DecodeUrlSafe(urlB64)
        val chacha = Cipher.getInstance("ChaCha20-Poly1305")
        chacha.init(Cipher.DECRYPT_MODE, SecretKeySpec(chachaKey, "ChaCha20"), IvParameterSpec(nonce))
        val intermediate = chacha.doFinal(ciphertext)

        return String(
            b64DecodeUrlSafe(String(swapPairs(intermediate), Charsets.ISO_8859_1)),
            Charsets.UTF_8
        )
    }

    // -- crypt5 helpers ------------------------------------------------------

    // String "ikdhaaoxaiilieih" reversed → "hieiliaoxaaohdki"; chunked by 2,
    // taking the second char of each pair → "iiioaodi". This selector is a
    // single fixed string in the current Happ build but we keep it data-driven
    // in case future builds change it.
    private const val GETHELP_ARG = "ikdhaaoxaiilieih"

    private fun deriveSelector(input: String): String {
        val rev = input.reversed()
        val sb = StringBuilder(rev.length / 2 + 1)
        var i = 0
        while (i < rev.length) {
            val end = minOf(i + 2, rev.length)
            sb.append(rev[end - 1])
            i += 2
        }
        return sb.toString()
    }

    private fun extractNonce(payload: String): String {
        if (payload.length < 16) return ""
        val n = payload.substring(4, 16)
        // Each 4-char block ABCD → CDAB
        return buildString(12) {
            append(n[2]); append(n[3]); append(n[0]); append(n[1])
            append(n[6]); append(n[7]); append(n[4]); append(n[5])
            append(n[10]); append(n[11]); append(n[8]); append(n[9])
        }
    }

    private fun extractUrlB64(payload: String, n: Int): String {
        if (payload.length < 20 + n) return ""
        return payload[17] + blockPairSwap(payload.substring(20, 20 + n), n - 1)
    }

    private fun extractEncStr(payload: String, n: Int): String {
        val urlEnd = 20 + n
        val encEnd = urlEnd + 684
        if (payload.length < encEnd) return ""
        val urlRegion = payload.substring(20, urlEnd)
        val encRegion = payload.substring(urlEnd, encEnd)
        val skip = ((n - 1) / 4) * 4 + 1
        val skipChar = if (skip < urlRegion.length) urlRegion[skip] else ' '
        return skipChar + blockPairSwap(encRegion, 683)
    }

    /** For each output position j (1-indexed): k=(j-1)/4, p=(j-1)%4, src=4k+(p+2)%4.
     *  Net effect on each 4-char block: ABCD → CDAB. Trailing chars beyond `length`
     *  are dropped. */
    private fun blockPairSwap(region: String, length: Int): String {
        val sb = StringBuilder(length)
        for (j in 1..length) {
            val k = (j - 1) / 4
            val p = (j - 1) % 4
            val src = 4 * k + (p + 2) % 4
            if (src < region.length) sb.append(region[src])
        }
        return sb.toString()
    }

    private fun swapPairs(bytes: ByteArray): ByteArray {
        val out = bytes.copyOf()
        var i = 0
        while (i + 1 < out.size) {
            val t = out[i]; out[i] = out[i + 1]; out[i + 1] = t
            i += 2
        }
        return out
    }

    // -- key plumbing --------------------------------------------------------

    private class Pkcs1Key(b64: String) {
        val key: PrivateKey
        val modulusBitLength: Int
        init {
            val pkcs1 = Base64.decode(b64, Base64.DEFAULT)
            val pkcs8 = wrapPkcs1AsPkcs8(pkcs1)
            val k = KeyFactory.getInstance("RSA").generatePrivate(PKCS8EncodedKeySpec(pkcs8))
            key = k
            modulusBitLength = (k as java.security.interfaces.RSAPrivateKey).modulus.bitLength()
        }
    }

    private val pkcs1Keys: Array<Pkcs1Key> by lazy {
        arrayOf(Pkcs1Key(KEY_CRYPT), Pkcs1Key(KEY_CRYPT2), Pkcs1Key(KEY_CRYPT3), Pkcs1Key(KEY_CRYPT4))
    }

    @Volatile private var expandedKeysCache: JSONObject? = null
    @Volatile private var selectorsCache: JSONArray? = null

    private fun loadCrypt5Assets() {
        if (expandedKeysCache != null && selectorsCache != null) return
        synchronized(this) {
            if (expandedKeysCache != null && selectorsCache != null) return
            val ctx = AngApplication.application
            val keysJson = ctx.assets.open("happ/expanded_rsa_keys.json").bufferedReader().use { it.readText() }
            val selJson = ctx.assets.open("happ/selectors.json").bufferedReader().use { it.readText() }
            expandedKeysCache = JSONObject(keysJson)
            selectorsCache = JSONArray(selJson)
        }
    }

    private fun expandedRsaKeyFor(selector: String): String? {
        loadCrypt5Assets()
        val keys = expandedKeysCache ?: return null
        val sels = selectorsCache ?: return null
        if (keys.has(selector)) return keys.getString(selector)
        for (i in 0 until sels.length()) {
            val triple = sels.optJSONArray(i) ?: continue
            val starts = triple.optString(0)
            val mid = triple.optString(1)
            val ends = triple.optString(2)
            if (selector.length >= 6 &&
                selector.startsWith(starts) &&
                selector.substring(2, 6) == mid &&
                selector.endsWith(ends)
            ) {
                val combined = starts + mid + ends
                if (keys.has(combined)) return keys.getString(combined)
            }
        }
        return null
    }

    private fun parsePkcs8RsaKey(pkcs8B64: String): PrivateKey {
        val der = Base64.decode(pkcs8B64, Base64.DEFAULT)
        return KeyFactory.getInstance("RSA").generatePrivate(PKCS8EncodedKeySpec(der))
    }

    /** Wrap a raw PKCS#1 RSAPrivateKey into a PKCS#8 PrivateKeyInfo so JCE can parse it. */
    private fun wrapPkcs1AsPkcs8(pkcs1: ByteArray): ByteArray {
        // AlgorithmIdentifier { OID 1.2.840.113549.1.1.1 (rsaEncryption), NULL }
        val rsaOidWithNull = byteArrayOf(
            0x30, 0x0D,
            0x06, 0x09, 0x2A.toByte(), 0x86.toByte(), 0x48, 0x86.toByte(), 0xF7.toByte(), 0x0D, 0x01, 0x01, 0x01,
            0x05, 0x00
        )
        val versionInteger = byteArrayOf(0x02, 0x01, 0x00) // INTEGER 0
        val privateKeyOctet = derOctetString(pkcs1)
        val body = versionInteger + rsaOidWithNull + privateKeyOctet
        return derSequence(body)
    }

    private fun derSequence(content: ByteArray): ByteArray = byteArrayOf(0x30) + derLength(content.size) + content
    private fun derOctetString(content: ByteArray): ByteArray = byteArrayOf(0x04) + derLength(content.size) + content

    private fun derLength(n: Int): ByteArray = when {
        n < 0x80 -> byteArrayOf(n.toByte())
        n < 0x100 -> byteArrayOf(0x81.toByte(), n.toByte())
        n < 0x10000 -> byteArrayOf(0x82.toByte(), (n shr 8).toByte(), n.toByte())
        else -> byteArrayOf(0x83.toByte(), (n shr 16).toByte(), (n shr 8).toByte(), n.toByte())
    }

    private fun b64DecodeUrlSafe(s: String): ByteArray {
        var t = s.replace('-', '+').replace('_', '/')
        while (t.length % 4 != 0) t += '='
        return Base64.decode(t, Base64.DEFAULT)
    }

    // -- PKCS#1 RSA private keys (base64, no PEM wrapper) --------------------
    // Sourced from https://github.com/LeeeeT/happ-decryptor/blob/main/src/decrypt.js

    private const val KEY_CRYPT = ""

    private const val KEY_CRYPT2 = ""

    private const val KEY_CRYPT3 = ""

    private const val KEY_CRYPT4 = ""
}
